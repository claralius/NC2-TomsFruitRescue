...
class Fruit: SKSpriteNode{ //SKSpriteNode used for graphical element. Fruit represents graphical element in SpriteKit
    ...
    init(image: String, isSpoiled: Bool) {
        ...
        super.init(texture: texture, color: .clear, size: CGSize(width: 100, height: 100)) // super karena manggil dari parent
        self.physicsBody = SKPhysicsBody(rectangleOf: self.frame.size) //using self karena ini bawaan dari SKSpriteNode yang udah diinherit sama Fruit class
...
class GameScene: SKScene{ // game background scene & handles logic and content
    ...
    override func didMove(to view: SKView) {
        physicsWorld.gravity = CGVector(dx: 0, dy: -0.65) //simulasi gravity. dx itu horizontal force & dy downward gravitational force
        backgroundColor = .clear
        generateFruit(duration: duration)
        gameData.backgroundMusic?.play()
    }
    
    func generateFruit(duration: Double){//sequence of actions to create fruits dan nunggu
